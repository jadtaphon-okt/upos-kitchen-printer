...
const install = Vue => {
    ...
    const registerInstance = (vm, callVal) => {
        ...
        if (isDef(i) && isDef((i = i.data)) && isDef((i = i.registerPrinterInstance))) {
            i(vm, callVal)
        }
    }
    Vue.mixin({
...
class PrinterWebSocket {
    ...
    connect() {
        return new Promise((resolve, reject) => {
            ...
            try {
                const ws = new Socket(`${this.protocol}${this.address}:${this.port}${this.prefix}`)
                /* eslint-disable */
                ws.onopen = e => {
                    this.heartCheck()
...
class IminPrinter extends PrinterWebSocket {
    ...
    constructor(url) {
        {
            warn(`Printer must be called with the new operator.`)
        }
        super(url)
    ...
    printAndFeedPaper(height) {
        this.send(this.sendParameter('', 4, height <= 0 ? 0 : height >= 255 ? 255 : height))
    }
    /**
    ...
    setAlignment(alignment) {
        this.send(this.sendParameter('', 6, alignment <= 0 ? 0 : alignment >= 2 ? 2 : alignment))
    }
    /**
     * Set text size
     ...
     */
    setTextStyle(style) {
        this.send(this.sendParameter('', 9, style <= 0 ? 0 : style >= 3 ? 3 : style))
    }
    /**
     * Set line spacing
     * @param {string} space
     ...
     */
    setTextWidth(width) {
        this.send(this.sendParameter('', 11, width <= 0 ? 0 : width >= 576 ? 576 : width))
    }
    /**
     * Print text
     * @param {string} text
        ...
        this.send(
            this.sendParameter(
                type !== void 0 && !type && text.charAt(text.length - 1) === 'n'
                    ...
                    : text + '\n',
                type !== void 0 ? 13 : 12,
                type !== void 0 ? (type <= 0 ? 0 : type >= 1 ? 1 : type) : void 0
            )
        )
    }
    /**
    ...
    setBarCodeHeight(height) {
        this.send(this.sendParameter('', 16, height <= 1 ? 1 : height >= 255 ? 255 : height))
    }
    /**
    ...
    setBarCodeContentPrintPos(position) {
        this.send(this.sendParameter('', 17, position <= 0 ? 0 : position >= 3 ? 3 : position))
    }
    /**
     * Print barcode
     ...
     */
    setQrCodeSize(level) {
        this.send(this.sendParameter('', 20, level <= 1 ? 1 : level >= 9 ? 9 : level))
    }
    /**
    ...
    setQrCodeErrorCorrectionLev(level) {
        this.send(this.sendParameter('', 21, level <= 48 ? 48 : level >= 51 ? 51 : level))
    }
    /**
    ...
    printSingleBitmap(bitmap, alignmentMode) {
        ...
        image.onload = () => {
            ...
            formData.append('file', dataURItoBlob(compressImg(image, dataURItoBlob(bitmap).type)))
            let XHR = null
